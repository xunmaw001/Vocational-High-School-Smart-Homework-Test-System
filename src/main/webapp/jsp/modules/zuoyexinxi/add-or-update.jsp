...
<body>
					...
					<div class="row breadcrumb-bar">
						...
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>作业信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑作业信息</span></li>
							</ol>
						</div>
					</div>
				</div>
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>作业文件</label>
										<div>
										<div class="upload">选择文件<input name="file" type="file" class="form-control-file" id="zuoyewenjianupload"></div>
										<label id="zuoyewenjianFileName"></label>
										<input name="zuoyewenjian" id="zuoyewenjian-input" type="hidden">
										</div>
									</div>
									<div class="form-group col-md-6">
										<label>要求时间</label>
										<div id="yaoqiushijianDate" class="input-append date form_datetime">
											<input id="yaoqiushijian-input" name="yaoqiushijian" size="15" type="text" readonly>
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "zuoyexinxi/info/" + id,
		...
		// 文件上传
		function upload(){
			$('#zuoyewenjianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {							
					document.getElementById('zuoyewenjian-input').setAttribute('value','file/download?fileName='+data.result.file);
					if(document.getElementById('zuoyewenjianFileName') != null){
						document.getElementById('zuoyewenjianFileName').innerHTML = "上传成功!";
					}		
					$("#zuoyewenjianImg").attr("src",data.result.file);												
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("zuoyexinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("zuoyexinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("zuoyexinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dateTimePick(){
			...
			$("#yaoqiushijianDate").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
		...
		function dataBind(){
			...
			$("#zuoyewenjian-input").val(ruleForm.zuoyewenjian);
			$("#zuoyewenjianFileName").html(ruleForm.zuoyewenjian);
			$("#yaoqiushijian-input").val(ruleForm.yaoqiushijian);
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.zuoyewenjian != null && res.data.zuoyewenjian != ''){

						$("#zuoyewenjian-input").val(res.data.zuoyewenjian);
						$('#zuoyewenjianFileName').val(res.data.zuoyewenjian)
					}
					if(res.data != null && res.data != '' && res.data.yaoqiushijian != null && res.data.yaoqiushijian != ''){

						$("#yaoqiushijian-input").val(res.data.yaoqiushijian);
						$('#yaoqiushijianFileName').val(res.data.yaoqiushijian)
					}
